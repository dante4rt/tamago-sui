...
import { queryKeyOwnedAccessories } from "@/hooks/useQueryOwnedAccessories";
import type { AccessoryListing } from "@/types/Marketplace";
...
export function useMutateCancelAccessoryListing() {
  ...
  return useMutation({
    ...
    onMutate: async ({ listingId }) => {
      ...
      await queryClient.cancelQueries({ queryKey: queryKeyAccessoryListings });

      // Snapshot the previous value
      const previousListings = queryClient.getQueryData<AccessoryListing[]>(queryKeyAccessoryListings);

      // Optimistically update to remove the listing
      queryClient.setQueryData<AccessoryListing[]>(queryKeyAccessoryListings, (old) => {
        if (!old) return old;
    ...
    onError: (err, _listingId, context) => {
      ...
      if (context?.previousListings) {
        queryClient.setQueryData(queryKeyAccessoryListings, context.previousListings);
      }
